...
class Talents:
    ...
    def player_speed(self): # max 20
        self.player_data["player_speed"] = round(self.player_data["player_speed"] + 0.5, 1)

    def player_size(self): # min 5
        self.player_data["player_size"] = round(self.player_data["player_size"] - 0.5, 1)

    def dash_cooldown(self): # min 30
        self.player_data["dash_cooldown"] = int(self.player_data["dash_cooldown"] - 5)
    ...
    def attack_damages(self): # infinit
        self.player_data["attack_damages"] = int(self.player_data["attack_damages"] * 1.1)

    def attack_speed(self): # max 10
        self.player_data["attack_speed"] = round(self.player_data["attack_speed"] + 0.1, 1)

    def stationary_threshold(self): # min 15
        self.player_data["stationary_threshold"] = round(self.player_data["stationary_threshold"]  - 0.3, 1)

    def projectile_size(self): # max 20
